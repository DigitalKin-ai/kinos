...
Responsabilités:
- Surveille les logs et les changements dans tous les documents
- Produit des synthèses concises des activités
...
class SuiviAgent(ParallagonAgent):
    ...
    def determine_actions(self) -> None:
        try:
            ...
            summary = self._create_summary(new_logs)
            if not summary:
                return

            # Ajouter le résumé au contenu existant
            timestamp = datetime.now().strftime("%H:%M:%S")
            new_entry = f"[{timestamp}] {summary}\n"
            
            # Ajouter au début du fichier
            current_content = self.current_content or ""
            self.current_content = new_entry + current_content
            
            # Sauvegarder dans le fichier
            with open(self.file_path, 'w', encoding='utf-8') as f:
                f.write(self.current_content)

            # Mettre à jour le timestamp du dernier log traité
            if new_logs:
                self.last_log_timestamp = new_logs[-1].get('timestamp')

        except Exception as e:
            self.logger(f"[{self.__class__.__name__}] ❌ Erreur: {str(e)}")

    def _get_new_logs(self) -> list:
        """Récupère les nouveaux logs depuis le dernier check"""
        if not self.logs_buffer:
            return []

        if not self.last_log_timestamp:
            self.last_log_timestamp = self.logs_buffer[0].get('timestamp')
            return []

        new_logs = []
        for log in self.logs_buffer:
            if log.get('timestamp') > self.last_log_timestamp:
                new_logs.append(log)
        return new_logs

    def _create_summary(self, logs: list) -> str:
        """Crée un résumé des activités basé sur les logs"""
        if not logs:
            return ""

        # Analyser les logs pour créer un résumé concis
        actions = []
        for log in logs:
            message = log.get('message', '')
            if "✓" in message:  # Succès
                actions.append(message.replace("✓", "").strip())
            elif "❌" in message:  # Erreur
                actions.append(f"échec: {message.replace('❌', '').strip()}")
            elif "📝" in message:  # Modification
                actions.append(message.replace("📝", "").strip())

        if not actions:
            return ""

        # Créer une phrase de résumé
        if len(actions) == 1:
            return actions[0]
        else:
            return f"{len(actions)} actions effectuées: {', '.join(actions[:3])}..."
